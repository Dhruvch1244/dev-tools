...
import { parseImportFile, type ImportedRequest } from '../lib/postmanImport'
...
export function PostmanImportPage() {
    ...
    <div className="flex h-full flex-col gap-3">
      <Panel>
        <div className="flex flex-col gap-3 p-4">
          <SectionLabel>Collection file</SectionLabel>
          <label className="flex cursor-pointer items-center gap-2 rounded-xl border border-dashed border-rule px-3 py-2.5 text-xs text-ink-soft transition-colors hover:border-cyan/40 hover:text-ink">
            ...
            <span className="truncate">{fileName ?? 'Choose a Postman collection (.json) or Insomnia export…'}</span>
            <input type="file" accept=".json" onChange={(e) => onFile(e.target.files?.[0] ?? null)} className="hidden" />
          </label>
          {requests.length > 0 && (
            <>
      ...
      <Panel className="flex flex-1 flex-col overflow-hidden">
        <div className="flex-1 overflow-auto p-3">
          {requests.length === 0 ? (
            <div className="flex h-full items-center justify-center text-sm text-ink-faint">Parsed requests will be previewed here before import.</div>
          ) : (
