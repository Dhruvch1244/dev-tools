...
export function Panel({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`rounded-[1.75rem] bg-white/[0.02] p-1.5 ring-1 ring-white/[0.06] ${className}`}>
      <div className="h-full rounded-[calc(1.75rem-0.375rem)] border border-rule-soft bg-surface shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
        {children}
      </div>
    </div>
  )
...
export function Button({
  ...
  const variants: Record<string, string> = {
    default: 'bg-white/[0.04] text-ink ring-1 ring-white/[0.08] hover:ring-white/[0.16] hover:bg-white/[0.06]',
